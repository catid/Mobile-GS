...
class GaussianModel:
    ...
    def load_ply(self, path):
        ...
        expected_extra_coeffs = 3 * ((self.max_sh_degree + 1) ** 2 - 1)
        if extra_f_names:
            assert len(extra_f_names) == expected_extra_coeffs
            ...
            # Pretrain-only PLYs may store DC terms only; keep higher-order SH terms zeroed.
            features_extra = np.zeros((xyz.shape[0], 3, (self.max_sh_degree + 1) ** 2 - 1), dtype=np.float32)
